...
## 📋一般質問ネタDB との統合（2026-04-30）
- **data_source_id**: `42716725-fece-497f-9782-705076539de4`
- インタビュー設計と**同時実行**で一般質問ネタDBへエントリ自動登録
- **状況=調査中**で登録（市民の声収集中の状態を表現）
- **ネタ元=市民相談＋現場で気づき**（AIインタビュー由来を市民相談系として整理）
- メモ／根拠に管理画面URL＋公開URLを記録
- council-material-creator が一般質問原稿化時に本DBエントリを参照→状況を質問案/提出/通告/実施へ進行

## ライフサイクル全体図
1. **ai-interview-config-designer**（テーマ→設定設計＋ネタDB登録）
2. 市民の回答収集（数日〜数週間）
3. **ai-interview-sns-poster**（回答→SNS投稿＋市民意見BOX登録）
4. **council-material-creator**（ネタDB状況更新＋一般質問原稿生成）
5. 議会本会議で質問→答弁→録画
6. **video-content-strategist**（ショート動画化）
7. **blog-writer**（議会報告ブログ）

→ 1テーマが「市民の声収集→議会質問→広報」まで全自動連動
