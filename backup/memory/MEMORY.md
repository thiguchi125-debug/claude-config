...
- [Notion内部リンクの真の落とし穴は表示テキスト](feedback_notion_link_deeplink.md) — 2026-05-01に2回検証。href は API が自動で app.notion.com/p/ に正規化するので無問題。本当の犯人は `[https://www.notion.so/xxx](href)` のように**表示テキスト**側にURL文字列が露出していた件。朝ダッシュボード等3週間分9ページ45リンクの表示テキストを通常文字列に修正済
