...
- [市民要望管理台帳(Googleシート)](project_yobo_sheet_citizen_requests.md) — **稼働中(9/5)**。市民要望は`yobo.py add`・朝は`yobo.py alert`(ohayo§9b)。📱今やることタブ＋自動ソート(9/5)。**次＝Todoist使い分け・一次保存先・独自アプリ可否の設計**
