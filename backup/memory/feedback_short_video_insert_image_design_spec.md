...
2. **字幕セーフ帯 y=1240〜1460px を全画像で固定統一**（※2026-07-02京都直通動画で草川承認の確定座標＝[[feedback-short-video-subtitle-safe-zone]]が正本。本仕様の旧y1150〜1470は広め運用で帯を内包するため互換だが、新規制作は1240〜1460で設計し上下ゾーンを広く使う。2026-07-04統一）。帯内は読ませ文字・主要素・建物輪郭・カード縁を置かず穏やかな一様背景のみ。実装は絶対座標固定・中央寄せ禁止。完成後、半透明赤帯オーバーレイのコンタクトシートでピクセル一致を必ず確認。
