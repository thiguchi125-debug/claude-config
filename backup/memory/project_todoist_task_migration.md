...
- **残作業**:
  1. **期限超過37件の棚卸し**（Notion由来で期限5月末〜6月上旬のまま。実は済の照会タスク多数想定）。「期限超過リスト出して」→ `td.py overdue`で一覧→完了/再設定をバッチ処理。
  2. **Notionタスクview 2ページ目（古い層・大半Done/Archive）**の未完了スイープ（カーソル期限切れのため要再query・南部地区/井田川地区市民相談の個別タスクはこの層にある可能性）。
  3. Todoist安定後、Notion✅タスクDB/🗂️PJDBをアーカイブ（草川判断・元データは保全）。
