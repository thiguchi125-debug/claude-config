---
name: feedback_thumbnail_ask_base_photo_first
description: ブログ/記事のサムネ（アイキャッチ）を作るときは、まず元になる草川の写真を求めてから作る
metadata: 
  node_type: memory
  type: feedback
...
ブログ・記事のサムネイル（アイキャッチ／OGP画像）を制作するとき、いきなりHTML/CSSのタイポグラフィだけで組む前に、**まず「元になる草川の写真はありますか？」と一度確認してから**作る。写真があればそれを主役に据えたデザイン、無ければ（草川OKなら）タイポ＋モチーフ主体のデザインに切り替える。
