...
## 4つの起動モード（PC側SKILL.md）
1. **記録モード**（既定）: 「記録：〇〇した」等で Step 1〜5 を実行
2. **統合仕上げモード**（2026-04-27変更・「仕上げ」単独の既定動作）: 「仕上げ」「仕上げて」で**仕上げモード(S1〜S4)＋コンテンツ抽出モード(C1〜C6)を同時実行**。U1並列スキャン→U2統合一覧提示（Part A=📱下書き／Part B=当日素材候補）→ユーザー選択→U3振り分け処理→U4統合サマリー。重複排除は📱優先でAに寄せる
3. **仕上げモード単独**: 「モバイル下書きを仕上げ」「📱だけ仕上げ」等で起動（S1〜S4のみ）
4. **コンテンツ抽出モード単独**（2026-04-27追加）: 「今日のコンテンツ抽出」「今日のネタ全部出して」「今日まるごと抽出」等で起動（C1〜C6のみ）。当日の活動ログ／市民意見／一般質問ネタ／ニュース／政策候補／完了タスクを横断スキャン→テーマ候補5〜10件提示→ユーザーGOサイン取得→content-pipeline委任で生成。下書きが無くてもゼロから発信ネタを発掘できる点が仕上げモードと異なる。市民相談個人情報秘匿・iJAMP転載禁止・公選法配慮を明文化済み
