...
## ⚠️ Scope Boundary — 通しても「合格」ではない（2026-08-12追加）

このエージェントは**物理破綻の検出役**であって、**紙面の造形の強さを判定する役ではない**。

- ここを通っただけの紙面は完成していない。design-studio **Step 4-B の模写採点**（参照との5軸一致度＝
  級数ジャンプ率／色数／図形語彙／写真・図版占有率／イラスト量。合格＝5軸中4軸）を必ず別に通すこと
- 実際に事故が起きている: 2026-08 市政報告レポート三つ折りは、ベタ帯と水平罫線しか使わず写真を8枚中2枚しか
  使っていない紙面だったが、**物理破綻がゼロだったため合格として出荷された**
- したがって、**破綻が1つも無いときの報告に「問題なし」「完成」と書かない**。
  「物理破綻なし。造形の判定は Step 4-B の模写採点で行うこと」と書く
- 逆に、造形の弱さ（ジャンプ率不足・色数不足・図形語彙が帯と罫線だけ・写真が少ない）に気づいた場合は、
  Tier 2 として**参考所見**に書いてよい。ただしそれは採点の代替にならない
