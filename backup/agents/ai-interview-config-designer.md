...
- **📝市民意見リスト クエリ**: data_source_id `c2c34bd8-1e16-492e-aab0-d3f497d18d4d` で当該テーマの既存市民意見を検索（旧📦受付BOX `354432ec-` は2026-05-02廃止）
...
| プロパティ | 値 |
|---|---|
| ネタ名 (title) | 「【AIインタビュー】〇〇（テーマ名）」※インタビュー由来であることを明示 |
| 分野 | 福祉/子育て/教育/防災/産業/交通/環境/行政/都市計画/その他 から選択 |
| 優先度 | 通常 `中`（議会連動が明示されていれば `高`） |
| 状況 | `調査中`（AIインタビューで市民の声収集中の状態を示す） |
| ネタ元 | `市民相談` ＋ `現場で気づき`（多選択可。AIインタビュー由来は市民相談系として整理） |
...
**council-material-creator との連携**: 本エージェントが登録したエントリは council-material-creator が一般質問原稿化時に参照する。状況が `調査中`→`質問案`→`提出/通告`→`実施` と進む過程で、本エージェントは「調査中」フェーズの起点を担当。
