...
## 📌 Step 7：Notionページ保存（必須）

議案カルテを返したあと、**必ず📅ミーティングノートDB配下にページを作成**して保存する。期別議会フォルダ（例: `2026.3議会`）配下には作らない。詳細手順は `~/.claude/projects/-Users-kusakawatakuya/memory/feedback_council_meeting_db_placement.md` を参照。

### 配置先
- **parent**: `{"data_source_id": "26d7848d-ad1a-4e84-806f-a8dbccb1872b"}`（📅ミーティングノートDB）
- **会議体マスタ**（`46414643-9717-404f-9572-2908c2c0a7f3`）に該当委員会／議会期がなければ即新規作成して relation 紐付け

### 標準プロパティ
| プロパティ | 値 |
|---|---|
| タイトル | `YYYY.M [会議体] [議題]（M/D議案分析）` 例: `2026.6 教育民生委員会 議第〇〇号 △△条例改正（6/15議案分析）` |
| 会議種別 | `議会公務` |
| ステータス | `開催前`（採決後に `開催済み`/`完了`） |
| 重要度 | `高` |
| フォローアップ要否 | `__YES__` |
| 作成元 | `手動` |
| 開催日 | 採決予定日（`date:開催日:start`） |
| 会議体 | 該当委員会／本会議の会議体マスタURL |
| 場所 | `亀山市役所` |
| 参加者 | `〇〇委員会 委員（草川）／執行部（担当課）` |
| アイコン | 📋（議案分析） |

### 本文構成
議案カルテの全文（メタ情報〜参照まで）に加えて、末尾に以下のメモ欄を必ず付ける：
...
### 注意
- agenda-analyzer は **議案カルテ生成 → ページ保存 → URL返却** までを1パスで実行
- 「議案を分析して」だけでも保存まで自動で進める（草川承認は不要、URLは事後通知）
- 複数議案一括分析モードでは、トリアージ表を1ページ＋確信度LOWの個別カルテをそれぞれ別ページで作成
- 関連relation（関連政策候補・関連一般質問ネタ）は分かる範囲で紐付け、不明な場合は空でOK
