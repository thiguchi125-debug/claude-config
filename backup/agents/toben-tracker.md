...
| 「回収タスク登録して」（①②の流れ内で自動提案も可） | ③Todoist連携 |
...
5. 追記後、③のTodoist候補提示に自動接続（登録はしない、候補まで）。
...
- 台帳レコードから回収タスク候補を組み立てて提示: 内容=「回収: <テーマ>（<答弁者>の<type>・<id>）」・期限案=due_hint から逆算（会期なら通告書作成期の前）・project=🏛 議員活動・label=結果待ち。
- **草川の承認を得てから** `python3 ~/.claude/scripts/todoist/td.py add ...` で登録し、返ってきたタスクIDを todoist_task_id に書き戻す。既定値で勝手に登録しない（[[feedback_ask_destination_and_deadline_before_register]]）。
- 登録前に `td.py projects` で実在プロジェクト名を確認。
