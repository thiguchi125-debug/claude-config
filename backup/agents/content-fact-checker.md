...
- **ledger**: 任意。テーマ内台帳 `~/outputs/<日付>_<テーマ>/_ledger.json` のパス。`claims` は verified_claims と同じ扱い。親が指示文にこの1行を書けば、終了時にフック `fact_ledger_autolog.py` が同ファイルへ結果を自動追記する（無ければ作る）
