...
### Step 3-B: PDFバイナリの強制抽出（重要）

**WebFetchがPDFを「バイナリで読めません」と返してきた場合、絶対に「検証不能」「ハルシネーション疑い」と判定してはならない**。WebFetchはバイナリPDFをローカル保存してパスを返すので、必ず以下の手順で全文抽出して検証する：

1. WebFetchの返り値に `[Binary content (application/pdf, X.XMB) also saved to /Users/.../tool-results/webfetch-XXXXX.pdf]` の行があるか確認
2. そのファイルパスに対して Bash で `pdftotext -layout "<path>" -` を実行（macOS には `/usr/local/bin/pdftotext` がインストール済み・poppler同梱）
3. 抽出結果（プレーンテキスト）に対して正規の数値・固有名詞検証を実施
4. 該当ページを探すには `pdftotext -layout "<path>" - | sed -n '180,400p'` などで範囲限定読み

**`-layout` オプションは必須**（表組み・列構造を維持したまま抽出。なしだと数値が混在して読めない）。

**過去の失敗事例（2026-05-04セッション）**：
- 内閣府個別避難計画推進全国協議会 資料2（shiryo2.pdf, 7.6MB）の数値「1,722団体・80%超240団体・三重県29市町中1団体」を、WebFetchが「PDFバイナリで読めない」と返したことで「ハルシネーション疑い」と誤判定
- 結果、メインClaude側が pdftotext で再抽出する追加往復が発生し、約100Kトークンの消費とユーザー体験の悪化
- **教訓**: PDFは「読めない」ではなく「**pdftotext で読む**」が標準。Step 3-B を必ず実行すること。

### Step 3-C: 議事録・会議録の検索

亀山市議会会議録検索システム（https://kameyama.gijiroku.com/）はWebFetch可能だが、検索結果ページはJavaScript依存のためBashの `curl` ＋検索URLパラメータの組み合わせで取得が必要なケースあり。検索できない場合は「議事録要再確認」フラグを立てて草川本人に確認を促す。
...
8. **PDFバイナリを「読めない」で済ませない**: WebFetchがPDFをバイナリ返却した瞬間に検証放棄するのは最悪手。必ずStep 3-Bの`pdftotext -layout`抽出フローを踏む。中央省庁・自治体の重要統計はほぼPDFで提供されており、ここをサボるとハルシネーション疑いの誤判定が量産される（2026-05-04 内閣府shiryo2.pdfで実際に発生）
