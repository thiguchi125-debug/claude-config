...
**【2026-06-14 Todoist一本化】次アクションのタスク登録先は Notion DBではなく Todoist（`td.py add`／該当の地区・イベント箱・期限つき）。市民意見の保存先は📝市民意見リスト `c2c34bd8`。CLAUDE.md『タスク・プロジェクトはTodoistに一本化』＋『情報入力の自動振り分け保存（スマートインテーク）』に準拠する。**
**【2026-09-05 市民要望は台帳へ】市役所（県・警察・自治会含む）へ照会→回答→相談者へ報告、という市民要望案件の次アクションは Todoist でなく Googleスプレッドシート「市民要望管理台帳」へ1案件1行で登録する（`python3 ~/.claude/scripts/yobo/yobo.py add --t 件名 --k 地区 --r 経路 --s 相談者 --c 連絡手段 --b 分類 --a 相手先 --na 次アクション [--due]`）。以下のTodoist登録手順（STEP5）は、台帳に載らない草川自身の議会・政策作業にだけ適用する。相談者の氏名・連絡先は台帳にのみ書く（Notion市民意見リストと二重にしない）。Apps Script未設定なら yobo.py が手順書を案内するので、その間は台帳の行内容を提示して草川に手入力を依頼する。詳細＝OPERATIONS.md【B】例外節**
...
#### 5-2: タスク構造の判断フロー（2026-04-24フラット2層運用 + 2026-05-01 A〜E基準）

登録先は**Todoistに一本化**（`python3 ~/.claude/scripts/todoist/td.py add`・旧Notion✅タスクDBは廃止・書込禁止）。**タスクは全て並列（フラット）で登録**し、タスク間の親子階層は作らない：

1. **単発・1ステップで完結するか？** → YES: Todoistに1件登録 / NO: 次へ
2. **複数ステップの連鎖か？** → YES: **各ステップを並列タスクとして複数件登録**。タスク名先頭に `[相談テーマ]` プレフィクスを付けてグルーピング表現
3. **プロジェクト化判定（基準A〜E）** → 以下1つ以上該当時は**Todoistサブプロジェクト**（🏛 議員活動配下等）で束ねる提案をする（登録前に `td.py projects` で実在名を確認し、既存箱がなければ新設を提案）：
   - **A**: 同一テーマで関連タスクが2件以上
   - **B**: 同テーマで散在タスク3件以上
   - **C**: 着手〜完了が2週間超
   - **D**: 複数ステークホルダー（市民・所管課・議会等）絡み
   - **E**: 「〜検討」「〜整備」「〜推進」等の継続性ある語感
   - 該当基準（A/B/C/D/E）と `由来=市民意見起点` はプロジェクト説明（desc）に記載
   - **対象外**: 単発返信完結タスク（軽量運用維持）
   - **重複判定回避**: 既存Todoistプロジェクト・タスクと突合し、同テーマの箱が既にあれば新設せずそこへ登録

**目安：**
- 「明日土木課に電話」「写真の受領依頼」単発 → Todoistに1件登録（プロジェクト化判定スキップ）
- 「担当課照会→現地確認→是正指導→議会報告」のような4ステップ連鎖 → 基準A,D該当 → **並列タスク4件**＋Todoistサブプロジェクトで束ね
- 条例制定・政策テーマなど長期・複数案件 → 基準A,C,D,E該当 → Todoistサブプロジェクト作成、各タスクを `--project` で紐付け（情報蓄積は🗂プロジェクト・プラットフォーム(ds=`8975c6a9`)のPJページへ）
...
返信で約束したこと・現状メモから抽出しました。登録しますか？（全てTodoistへ・`td.py add`）

## タスク一覧（並列・フラット）
① [タスク名] | 優先度: 高/中/低 | 期限案: YYYY-MM-DD（未指定は今日+3日） | 登録先: 〔プロジェクト名〕
② [タスク名] | ...
③ [タスク名] | ...

## 🗂️ プロジェクト化の提案 ※複数ステップ・長期案件の場合のみ
Todoistサブプロジェクト「{テーマ}」（🏛 議員活動配下）を作成し、上記タスクをまとめますか？
...
同意が得られたタスクのみ **Todoist** に登録する（旧Notion✅タスクDB `292cf503-a68f-81c6-b9dd-000b3ffdd2ce` は廃止・書込禁止）：

```
python3 ~/.claude/scripts/todoist/td.py add "[相談テーマ] アクション" --project <箱名> --due YYYY-MM-DD --priority <1-4>
```

**並列タスクの登録（全ケース共通）：**
- 内容: タスク名（25字以内目安、複数ステップなら `[相談テーマ] アクション` の形式）
- `--priority`: 高=4／中=3／低=2（4が最高）
- `--due`: 草川が承認した期限（未指定なら今日+3日を提案して承認を得る）
- `--project`: 該当箱（登録前に `td.py projects` で実在名確認。既定は 🏛 議員活動）
- `--desc`: 市民意見リストページのURLを参照として記載。**相談元の氏名・連絡先はTodoistに書かない**（個人情報は📝市民意見リスト側にのみ保持）
- 相手のボール（返答待ち）になるタスクは `--label 結果待ち`

**プロジェクト化（複数ステップ・長期案件・ユーザー同意後のみ）：**
1. Todoistサブプロジェクト（🏛 議員活動配下）を作成（td.pyにプロジェクト作成cmdは無いため Todoist MCP `add-projects` または API `/api/v1/projects` で作成）。説明に該当基準（A〜E）と `由来=市民意見起点` を記載
2. 各並列タスクを `--project` で新プロジェクトに登録（登録済みタスクは Todoist MCP で移動）
3. 経緯・資料の蓄積が要る案件は🗂プロジェクト・プラットフォーム(ds=`8975c6a9`)にPJページも作成（Todoist=実行／Notion=蓄積）

**禁止事項：** タスク間の親子階層（親タスク／サブタスク）は絶対に作らない。旧🗂️プロジェクトDB（`292cf503-a68f-81fe-bd40-000b64314f2e`）は廃止・書込禁止（過去データの参照のみ可）。
...
✅ 登録完了
   - 市民意見リスト（Notion）: [URL]
   - Todoistタスク: N件（並列フラット）
     - [タスク名] [優先度] | 期限 | 〔登録先プロジェクト〕
   - Todoistプロジェクト（束ね用・該当時のみ）: [プロジェクト名]
```
...
全てTodoistに登録（`td.py add`・期限未指定は今日+3日案）

## タスク（並列・フラット）
① [タスク名] | 優先度: 中 | 期限案: YYYY-MM-DD | 〔登録先プロジェクト〕
② [タスク名] | 優先度: 高 | 期限案: YYYY-MM-DD | 〔登録先プロジェクト〕
③ [[相談テーマ] タスク名] | 優先度: 中 | 期限案: YYYY-MM-DD | 〔登録先プロジェクト〕
...
- Schema keys: `件名` (title), `意見内容`, `date:受付日:start`, `経路`, `分類タグ`, `対応状況`, `担当課`, `次アクション`, `相談者（氏名等）`, `連絡先`, `匿名`, `関連自治会_意見`(relation→自治会別訪問管理DB)

**🏘️ 自治会別訪問管理DB**（市民相談の自治会自動紐付け先）
- data_source_id: `996b1d15-2454-4667-9702-05d6d0faa75c`
- 相談者の所在地・意見内容から自治会名/町名/地区名を検出 → 自治会DBレコード検索 → 市民意見DB側`関連自治会_意見`relationに紐付け＋自治会DB側ページ本文「## 🤝 関係性メモ → ### 直近の場での発言」に時系列追記
- 検出ルール: nichijo Step 2.6-A の48自治会名＋22地区名＋施設名リスト

**✅ Todoist** — 全タスク・プロジェクトの統合登録先（2026-06-14一本化）
- エンジン: `python3 ~/.claude/scripts/todoist/td.py`（add/today/overdue/audit/list/done/rm/projects）
- プロジェクト構成: Inbox／🏛 議員活動／📋 政策・一般質問／📣 発信／🏡 家族・プライベート ほか案件別サブPJ（登録前に `td.py projects` で実在名確認）
- 状態管理はステータスでなく **期限＋ラベル**（@結果待ち／@保留／@アイデア／@読む）
- 登録は必ず草川承認後（保存先＋期限案を提示→回答を得てから `td.py add`）

**~~✅ タスクDB~~（廃止・2026-06-14 Todoist一本化。書込禁止・過去データの参照のみ可）**
- data_source_id: `292cf503-a68f-81c6-b9dd-000b3ffdd2ce`
- 旧Schema keys（参照時用）: `タスク名` (title), `ステータス` (`"inbox"` / `"Wish List"` / `"Done"` / `"Archive"`), `優先度`, `date:期限:start`, `プロジェクト`, `相談元氏名`, `相談元連絡先` ほか

**~~🗂️ プロジェクトDB~~（廃止・Todoist一本化。書込禁止・過去データの参照のみ可。現役のPJ情報蓄積は🗂プロジェクト・プラットフォーム ds=`8975c6a9`）**
- data_source_id: `292cf503-a68f-81fe-bd40-000b64314f2e`
- 旧Schema keys（参照時用）: `プロジェクト名` (title), `ステータス`, `優先度`, 期間, 要約, `タスク`, `進捗率`

---
## 📌 恒久ガードルール（実運用フィードバック・2026-09-05に memory から吸収。iken／aisatsu-prep／申込対応系も本節を正本として参照）

### 返信文でのおうむ返し（事実反復）禁止 — 全返信・応答系に適用
相手が書いてきた事実情報（日付・年齢・固有エピソード・行為）を返信文の中で**そのまま繰り返さない**。「読みました」アピール構文はAI生成物の典型的悪パターンで、わざとらしく不誠実に映る（草川2026-05-10指摘。NG実例＝「5月6日から8日、東京の実の娘さんと再会され…」「92年の長いご人生を歩まれたお姑様」「医療センターを退院されてから安全の里に入所され…」）。
- **作り方**: ①読んだ自分の心情/身体反応から書き始める（「『看取り』の二文字を読んで、しばらくスマホを置きました」「少し手が止まりました」）②その状況への自分の見解・決意・約束を組む ③固有名詞・数字を入れるなら自然な脈で1〜2点まで（4要素以上は反復臭）④短い方が安全 ⑤間接参照を使う（「あの停留所の話」「そのお時間に」「文面を読みながら」）
- 適用範囲: 市民意見返信A/B/C・相談メール下書き・SNSコメント返信・引用RT・お礼DM・LINE個別返信・会話応答。ai-interview-sns-poster でも市民の声セリフ引用以外の事実反復は控える

### SNS DM返信の確定スタイル（2026-06-18 部活動地域移行DMの草川手直し確定版から抽出）
- **議員の無知を露呈する表現は禁止**。「これは私も初めて知った大事な事実です」型NG。市民が持ち込んだ現場情報は「とても大事な現場の課題です」と価値を認めて受ける（知らなかったとは言わない）
- **返信が遅れたら挨拶直後に素直にお詫び**（「申し訳ございません、リクエスト欄に埋もれていて返信が遅れました」型）
- **SNS DMは末尾に署名を付けない**（誰からか自明）。署名フルはメール返信のみ（「経過報告」型メールの体裁は memory/feedback_citizen_reply_progress_report_style.md）
- 議会名を明示: 「本会議で」でなく「亀山市議会の本会議で」
- 継続フォローは「議会で継続して追いかけます」（「委員会の場で」より本人は「議会で」を好む。特定議会×対決動詞は使わない）
- 一人称「私」。共感→現状認識→約束（持ち帰り確認＋継続フォロー）→未来。約束はL2相応に留め個別即解決を約束しない

### フォーム自由回答の属性帰属は運営文脈（operational context）で判定する
主語の無い属性（アレルギー・体調・配慮事項）は、**誰が食べる／使う／対象か**から自然な主体を選んで即対応する。「推定するな」ではなく「文脈で確定できるときは確認不要、本当に不明なときだけ1行確認」が正（2026-05-19「温泉で、産後ケア。」申込のフォーム回答「えびかに」を①子に機械的に帰属→②指摘後に「どなたの分か要確認」と過剰是正、の二重ミス。草川「親に決まっている。確認はいらん、お弁当に配慮する文章だけ入れて」）。
- お弁当・大人用品 → 申込本人（基本お母さま）／離乳食・おやつ持込・子ども用品 → お子さま／託児中の食材接触 → 同伴お子さま全員／託児・保健師相談 → お子さま中心
- 判定したら即運用対応（該当食材除去・保健師申し送り）＋「対応します」と返信。「念のため」「気になりまして」型の確認段落は禁止。確認するなら本当に必要な1点だけ
