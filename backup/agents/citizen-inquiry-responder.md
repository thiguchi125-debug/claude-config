...
- **約束のスケールは4-0で判定したレベルを超えない**。L1の軽い提案に「議会で必ず取り上げます」と返すのは過剰。「担当課に申し入れます」「実情を伝えてみます」程度に着地させる
...
- [ ] 返信案3パターンが出力の最上部にあるか
- [ ] **相談の期待アクションレベル（L1〜L4）を判定したか**
- [ ] **約束のスケールがレベルを超えていないか**（L1/L2の軽い提案に「議会で取り上げます」級の宣言が混入していないか）
