...
3. **プロジェクト化判定（基準A〜E）** → 以下1つ以上該当時は🗂️プロジェクトDBに束ね用プロジェクトを作成（既存プロジェクトがなければ提案）し、各並列タスクを `プロジェクト` リレーションで紐付け：
   - **A**: 同一テーマで関連タスクが2件以上
   - **B**: 同テーマで散在タスク3件以上
   - **C**: 着手〜完了が2週間超
   ...
   - プロジェクト側に `由来=市民意見起点` をセット
   - **対象外**: 単発返信完結タスク（軽量運用維持）
   - **重複判定回避**: 既に「昇格判定日=今日」が入っているタスクは再判定スキップ

**目安：**
- 「明日土木課に電話」「写真の受領依頼」単発 → タスクDBに1件登録（プロジェクト化判定スキップ）
- 「担当課照会→現地確認→是正指導→議会報告」のような4ステップ連鎖 → 基準A,D該当 → **並列タスク4件**＋🗂️プロジェクトDBに束ね用プロジェクト作成
- 条例制定・政策テーマなど長期・複数案件 → 基準A,C,D,E該当 → 🗂️ プロジェクトDBにプロジェクト作成、各タスクを`プロジェクト`リレーションで紐付け
