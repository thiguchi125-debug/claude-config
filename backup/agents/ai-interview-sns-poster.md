...
### Step 6: 📣SNS投稿管理DB への自動登録（必須）
SNS投稿原稿そのものを Notion 📣SNS投稿管理DB へ登録し、投稿履歴・ステータス追跡を行う。Step 5 の📋市民意見受付BOXとは別レコードで、投稿原稿マスタとして残す。
...
| プロパティ | 値 |
|---|---|
| 投稿タイトル (title) | 「【AIインタビュー】〇〇（テーマ要約）」 |
| ステータス | `未着手`（生成直後・未投稿）※ Step 5の市民意見BOXとステータス意味が異なる点に注意。投稿実施後に手動で `完了` 更新。MEMORY.md `feedback_blog_sns_db_status_options.md` 参照（「下書き完成」選択肢は存在しない） |
| プラットフォーム (multi_select) | ユーザー指定のプラットフォーム1〜複数（例: `["Instagram"]` `["X","Facebook"]`）。指定がなければ Instagram/X/Facebook の3つを既定セット |
| ネタ元 | `市民相談`（AIインタビュー＝市民の声起点として固定） |
| 分野 | テーマに応じて 福祉/子育て/教育/防災/産業/交通/環境/行政/都市計画/その他 から1つ |
| 投稿予定日 (date:start) | 当日（ISO 8601 date） |
| メモ | 「AIインタビュー由来。提案者属性: 〇〇。📋市民意見受付BOX 受付ID: 〇〇（リンク）」 |

**content（ページ本文）**: SNS投稿原稿の全文（区切り線・空白行・絵文字をそのまま保持）。省略・要約禁止。

**登録タイミング**: Step 5 の直後に連続実行。両方完了後、ユーザーへ以下のように2行で報告:
```
📋市民意見受付BOXに登録: <URL>
📣SNS投稿管理DBに登録: <URL>
```

**重要**: プラットフォームが複数の場合も、レコードは**1件にまとめる**（multi_select で複数選択）。プラットフォームごとに原稿が異なる場合のみ複数レコード化を検討するが、本エージェントは原則1原稿1レコード。
