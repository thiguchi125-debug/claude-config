...
### Step 6: 📣SNS投稿管理DB への自動登録（必須・プラットフォーム別1レコード）

**生成した各プラットフォーム原稿を、1プラットフォーム=1レコードとして登録する。**

理由: 各プラットフォームで原稿が異なるため、別レコードで管理する。投稿ステータス・予定日・反響メモも個別追跡できる。

**登録先**: data_source_id = `1bd98deb-624f-402c-aeb3-bdaa4782b389`

**レコード作成ルール**:

| 生成バリアント | プラットフォーム (multi_select) | レコード数 |
|---|---|---|
| Instagram版 | `["Instagram"]` | 1件 |
| Facebook版 | `["Facebook"]` | 1件 |
| Threads版 | `["Threads"]` | 1件 |
| X版 | `["X"]` | 1件 |

→ デフォルト4プラットフォーム全生成なら **計4レコード**作成。  
→ 部分指定なら指定分のみ。
...
| プロパティ | 値 |
|---|---|
| 投稿タイトル (title) | **30字以内**で「【AIインタビュー#NN】〇〇テーマ／〇〇用」形式（例: `【AIインタビュー#24】部活動地域移行／Instagram用`）。SNS本文の見出しを流用しない |
| ステータス | `未着手`（生成直後・未投稿）。投稿実施後に手動で `完了` 更新。MEMORY.md `feedback_blog_sns_db_status_options.md` 参照 |
| プラットフォーム (multi_select) | 媒体に応じて 1つ（multi_select だが本エージェントは原則1値） |
| ネタ元 | `市民相談`（AIインタビュー＝市民の声起点として固定） |
| 分野 | テーマに応じて 福祉/子育て/教育/防災/産業/交通/環境/行政/都市計画/その他 から1つ |
| 投稿予定日 (date:start) | 当日（ISO 8601 date） |
| メモ | **100字以内**で「提案者: 〇代・〇〇。文字数: 〇〇字。📋意見BOX: 〇〇」形式。冗長な事例列挙・セッションIDなどはcontent末尾に注記 |

**content（ページ本文）**: その媒体の原稿全文（区切り線・空白行・絵文字をそのまま保持）。省略・要約禁止。  
content末尾に注記ブロックを追加:
```
---
[meta] AIインタビュー#NN（YYYY-MM-DD）／媒体: 〇〇／文字数: 〇〇字（上限: 〇〇字）／セッションID: 〇〇 ／ 関連: 📋意見BOX <URL> / 📝質問ネタDB <URL>
```
...
- [ ] **📣SNS投稿管理DB に登録済みか？**（媒体ごとに別レコード、プラットフォーム multi_select は1値）
