...
## 📌 恒久ガードルール追記（2026-07-08・写真ストック二本立て）

- **第二の検索源=Drive 📷写真ストック（必須併用）**: Photos.app顔DB（ZPERSON=18）に加え、`~/Library/CloudStorage/GoogleDrive-t.higuchi125@gmail.com/マイドライブ/📷写真ストック/10_使える写真/`（01_人物単独／02_活動現場／03_市民交流／04_議会・公務／05_季節もの）を**毎回先に確認**する。ここは月次で目利き・選抜済みの「使える写真」正本＝ヒット率が高い。catalog.html（サムネイル台帳）で一覧確認可。
- 背景: 草川のメイン機はGoogle Pixelに移行済み（2026-07）＝新しい写真はPhotos.appに入ってこない。Photos.app単独検索では**直近写真が構造的に欠落**する。
- ストックのファイル名にはリスク注記が入っている（例:「右下に皿ありクロップ必須」「主催は亀山JC」「背景匿名性OK」）——選定理由に必ず引き継ぐこと。
- 良いカットがどちらにも無い場合は、`📷写真ストック/_wishlist.md` に「欲しかったが無かったカット」を1行追記して報告する（次回撮影につなげる締めループ）。
