...
5. **動画生成（既定で続行・追加入力不要）** — 選択デザイン＋元写真→ffmpegで**合計10秒以内**の 9:16 mp4（写真1枚6.5s/2枚3.4s/3枚2.4s＋エンドカード2.5s。よほどの品質でなければ10秒超は完視聴が落ちる＝2026-07-05草川指示）。ズーム（Ken Burns）→見出し段階表示→エンドカード。エンドカードは**拳ポーズ公式写真をライム枠のフォトカードに額装（assets/kusagawa_portrait_src.jpg）＋スローガン「声をチカラに」（または「ええやん亀山」）＋#ええやん亀山**。実写のAI改変・白背景切り抜きはしない（襟と背景が同じ白で破綻＝2026-07-05検証）。句読点付き「声を、チカラに。」は使わない。無音。「画像だけ」指定時はスキップ。
