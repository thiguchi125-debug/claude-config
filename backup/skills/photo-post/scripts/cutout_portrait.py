...
def main():
    ...
    # 胴中央の台形保護帯（シャツV→拳へ下方に広がる）。明るいシャツは背景と
    # 画素値が近く値では分離できないため、この帯は値に関わらず不透明で守る。
    # 上辺は襟元に合わせて細く、下辺は肩幅に広げる。肩脇の白背景ギャップ
    # (y上寄り・左右に張り出す)は台形の外なので通常どおり抜ける。
    cx = w * 0.47
    py0 = h * 0.64
    hw_top = w * 0.10   # 上辺の半幅（襟元）
    hw_bot = w * 0.20   # 下辺の半幅（拳・肩）
    for y in range(h):
        if y >= py0:
            t = (y - py0) / (h - py0)
