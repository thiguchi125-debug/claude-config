---
name: oyasumi
description: 毎晩「おやすみ」「おやすみなさい」「good night」「夜のまとめ」「就寝」「1日の振り返り」「夜のブリーフィング」などと入力されたら必ず起動する。1日の活動（SNS投稿・ブログ・タスク完了・市民相談・ニュース・政策候補追加・Drive新規資料）を機械集約してNotionデイリーサマリを生成し、Google Driveに当日新規保存された資料を要約して📂Drive資料サマリDBへ蓄積（重複除外）、未完了タスクをTodoist（td.py）で集計整理（期限は据え置き・繰越判断は翌朝morning）、明日のスケジュールをプレビューし、朝のダッシュボードに「昨夜のまとめ」セクションを反映する就寝時オートメーション。金曜日の夜のみ追加で週次レポートを自動生成する。完全自動化（途中で選択肢を求めない）。
---
...
## 🔴 タスクはTodoist（2026-06-14一本化・2026-07-04本文修理済み）
本スキル内の「完了タスク集計・未完了タスクの確認・プロジェクト化判定」のデータソースは **すべてTodoist**（エンジン＝`python3 ~/.claude/scripts/todoist/td.py <cmd>`・token=`~/.config/todoist/token`・API必ず `/api/v1/`）。Notion✅タスクDB(292cf503)・🗂️プロジェクトDBは**廃止済み＝新規書込・定常クエリ禁止**（過去データの参照のみ可）。
- **今日完了したタスク** ＝ その日 `td.py done <id>` でcloseした分（会話・nichijoで完了にしたもの）。Todoistの完了履歴に依存せず、当日closeした件名を要約に載せる。
- **未完了タスクの確認** ＝ `td.py overdue`（期限切れ）＋`td.py today`（本日期限）。期限の付け替えは**oyasumiでは行わない**（据え置きで超過証跡保持→翌朝 `td.py morning` で草川が繰越/据え置きを判断・承認分のみ付け替え）。
- **監査・プロジェクト化判定** ＝ `td.py audit`（期限超過/期限なし/@結果待ち/@保留の件数）。基準A〜Eの判断ロジックは継続するが登録先はTodoistプロジェクト。
- **タスクの新規登録はoyasumiでは行わない**（承認キュー化・2026-07-03確定）。抽出した候補はデイリーサマリに載せ、翌朝ohayoで草川承認後のみ `td.py add`。
- 🗓 **例外的に期限つきで登録・期限を後付けする場合は task-add の突合手順を通す**（想定所要を1hセッション換算→カレンダー突合→✅/⚠️/🚫判定→承認）。突合なしの `td.py add --due`、`update-tasks` での期限後付け、`reschedule-tasks` での明後日以降への期限設定は PreToolUse hook が deny する。期限なしの登録はこれまで通り。朝の繰越（今日・明日への移動）はゲート対象外。
詳細は CLAUDE.md「タスク・プロジェクトはTodoistに一本化」＋ `project_todoist_task_migration.md`。
...
- **Drive `list_recent_files` は `pageSize: 10`**：議員業務関連の当日新規は通常3〜5件。10件超は modifiedTime 降順先頭10件のみ採用
...
**🔴 タスクデータはNotionではなくTodoist（2026-06-14一本化）**。上記Notion取得と並列で Bash を1回：

```bash
python3 ~/.claude/scripts/todoist/td.py today && \
python3 ~/.claude/scripts/todoist/td.py overdue && \
python3 ~/.claude/scripts/todoist/td.py audit
```

- **今日完了したタスク** ＝ 当日セッション・nichijoで `td.py done <id>` した件名を会話文脈から集約（把握できなければ「集計対象外」と記載。旧✅タスクDBはクエリしない）
- `today`/`overdue` の結果が後続 Step 3・Step 6 の「未完了・期限超過」の唯一のソース

**🚨 Notion側の取得手段は `notion-query-database-view` 一択（2026-05-01 燃費精査・改訂）**：
...
| DB | view_url |
|----|----------|
| ~~✅タスクDB~~（廃止・2026-06-14 Todoist一本化。参照のみ可・定常クエリ禁止） | `https://www.notion.so/292cf503a68f814eafcdec9eed56f273?v=292cf503a68f811e9555000c1fce8ad5` |
...
| ~~プロジェクトDB~~（廃止・Todoist一本化。参照のみ可・定常クエリ禁止） | `https://www.notion.so/292cf503a68f813aa68dd75cbc469c92?v=292cf503a68f814c914e000c76ddc78f` |
| 📂Drive資料サマリDB | `https://app.notion.com/p/8815412449ad4de5b3b57145513309df` |
| 🎯政策候補DB | 初回 `notion-fetch` で取得後 memo |

> ⚠️ **2026-05-08確定**：旧記載 `70eeaeabe7e246599e0e7d5f8fef88c6` は実は「📦 旧受付BOX（2026-04-30〜2026-05-02・市民意見リストへ統合済み・data_source 354432ec-）」を指していた廃止DB。現役は c2c34bd8 系。SNS投稿管理DB／一般質問ネタDB はpage URLのみで `?v=` 不在のため `notion-query-database-view` がvalidation_errorで0件返却していた（2026-04-30〜05-07の3DB失敗事故の根本原因）。
> ⚠️ **旧ブログ記事管理DB（dcdf44af-）は2026-05-03廃止統合済み**。SNS投稿管理DB（1bd98deb-）にプラットフォーム=ブログとして統合されている。

##### タスクデータ取得後の処理（必須・Todoist）

1. `td.py overdue` の結果 ＝「未完了・期限超過」（Todoistは完了済みを返さないので Done/Archive 除外処理は不要）
2. `td.py today` の結果 ＝「本日期限の未完了」
3. `td.py audit` の件数（期限超過/期限なし/@結果待ち/@保留）を Step 3 のプロジェクト化判定と Step 6 サマリへ受け渡す
...
- 今日のSNS投稿（1-D）
- 今日のブログ公開（1-D）
- 今日完了したタスク（Todoist・1-Dのタスクデータ取得より）
- 今日新規追加された市民意見（1-D）
...
### Step 3: 未完了タスクの整理 ＋ プロジェクト化判定（2026-05-01追加・2026-07-04 Todoist化）

#### 入力データの取得（必ずTodoist実クエリ）

Step 1-D で実行済みの Todoist クエリ結果を使う（未実行ならここで実行）：

1. **期限超過**: `python3 ~/.claude/scripts/todoist/td.py overdue`
2. **本日期限**: `td.py today`
3. **期限なし（inbox的）**: `td.py audit` の「期限なし」件数のみ使用（繰越し対象外・Step 6 サマリの 📋 積み残しでだけ言及。全件名が要る場合のみ `td.py nodue`）

#### プロジェクト化判定（Step 3 内に組込み・2026-05-01以降必須）

超過・本日期限のタスク群に対して、以下基準A〜Eのいずれか1つ以上該当する**プロジェクト未所属タスク**（Todoist Inbox 直下）を検出：
- **A**: 同一テーマで関連タスク2件以上 / **B**: 同テーマ散在10件以上
- **C**: 着手〜完了が2週間超（=繰越超過状態） / **D**: 複数ステークホルダー絡み
- **E**: 「〜検討」「〜整備」「〜推進」等の継続性ある語感

**重複判定回避**: 当日の nichijo 整理モード等で既にプロジェクト化判定済みのタスクは再判定スキップ（ダブル提案防止）。

該当検出時は朝のダッシュボード「💡 プロジェクト化候補」セクションに反映（草川が朝確認時にyes/no判断）。**oyasumi本体では自動登録しない**（朝の判断に委ねる。登録する場合の登録先はTodoistプロジェクト＝🏛議員活動配下サブプロジェクト等。旧🗂️プロジェクトDBは使わない）。

#### 繰越しルール（据え置き原則）

- **oyasumiではTodoistの期限を書き換えない**（期限超過は超過証跡としてそのまま保持）
- 繰越し（`--due today` 付け替え）は**翌朝 `td.py morning` で草川が超過一覧を見て判断**し、承認分のみ実施する（勝手に付け替えない）
- **タイトルへの ⚠️ プレフィクス書き込みは原則禁止**（タイトル汚染リスク・二重マーカー化）
  - ⚠️ は **ダッシュボード描画時に表示のみで付与** する（ohayo側のテーブル組み立てで処理）
  - 例外：ユーザーが明示的に「タイトルにも書いて」と指示したときのみ実施

#### 制約

- **絶対にタスクを削除しない**（`td.py rm` をoyasumiで実行しない）
...
Notion AI Meeting で当日記録された会議を、**📅 ミーティングノートDB（`26d7848d-ad1a-4e84-806f-a8dbccb1872b`）へ自動転記**する。これは nichijo整理モード R1-B / R2 / R4-A の subset を oyasumi 内部で実行するもので、ユーザーが明示的に「整理」と打たなくても夜の段階で自動整理が完了するようにする。
...
#### 4.5-8. フォローアップ候補抽出 → 承認待ちキュー（2026-07-03改訂・自動登録廃止）

> **旧仕様（〜2026-07-02）は候補を勝手にタスク登録していた**（「フォロータスク5件自動登録」と内訳なしで報告→草川が何が登録されたか分からない事故）。さらに登録先が廃止済み✅タスクDB(292cf503)のままだった。**v2からは登録せず、朝の承認に回す**。

4.5-3 で `フォローアップ要否=__YES__` と推定された AIミーティングノートに対して、本文先頭1500字を `notion-fetch` で取得し、以下のキーワードパターン行をパースしてタスク候補を抽出：
...
**展開ロジック（v2）**：
1. 1ミーティングから**最大5候補**まで抽出（超過は切り捨て・ノイズ防止）
2. **どこにも登録しない**。デイリーサマリ（Step 6）に以下のセクションを追加するだけ：
   ```
   ## 🧾 タスク登録候補（承認待ち・自動抽出）
   - [ ] 〈会議名〉: 〈候補文 60字以内〉（期限候補: MM/DD・推奨project: 🏛議員活動 等）
   ```
3. 重複防止: 既存Todoist（`td.py list`相当の把握があれば）・既存候補と同文はスキップ
4. **翌朝ohayoの§2がこのセクションを検出→草川に全件提示→承認分のみ `td.py add`**（登録先は必ずTodoist・廃止✅タスクDBは使わない）

**集計を Step 6 に渡す**：`mtg_extracted_tasks_total`（候補件数）と候補一覧。サマリ本文には「自動登録N件」という表現を**使わない**（登録はしていないため）。
...
**制約**：
1. Drive資料1件から**最大2展開**まで（質問ネタ＋政策候補の組合せ等）
...
## 📊 今日の数字（Todoist＋Notion実クエリ集計）

| 指標 | 件数 |
|------|------|
| ✅ 本日 `td.py done` でcloseしたタスク | N件 |
| 🚚 期限≤今日の未完了タスク（`td.py overdue`＋`today`） | M件（4/27期限 X件 / 4/28 Y件 / 4/29 Z件 など内訳） |
| 📆 明日（YYYY-MM-DD）期限のタスク（`td.py week` 先頭日） | 件数 |
| 📱 SNS投稿 | X件 |
| 📝 ブログ公開 | Y件 |
| 🎙️ 受付市民相談 | Z件 |
| 🗞️ 今朝のニュース | P件 |
...
**注意**: ダッシュボードの旧タスクテーブルからは件数を引かないこと。タスクは `td.py`（Todoist）、その他は実DBを必ずクエリして集計（2026-04-30事故踏まえ）。
...
## 🚚 明日へ持ち越すタスク（期限据え置き・翌朝 `td.py morning` で繰越判断）
...
## ✅ 自動抽出されたタスク候補（未確認・承認待ちキュー・2026-07-03改訂）

- **総件数**: `mtg_extracted_tasks_total` 件（ミーティング由来）
- **どこにも登録していない**。朝のohayoで一覧表示→草川承認分のみ `td.py add` でTodoist登録（旧✅タスクDBは使わない）
...
1. ⚠️期限超過タスクが N 件 → 翌朝 `td.py morning` の超過ブロックで繰越/据え置き判断
...
90%未満のスコアの下書きは**何もしない**（草川オリジナル原文のまま保持）。朝のohayo「📱下書き仕上げ保留」セクションで一覧表示し、草川が手動で「仕上げて」発話するかオリジナルのまま使うかを判断。
...
| ToDo・期限・次アクション | Todoist登録**候補**（自動登録禁止。Step 6 サマリ「✅ 自動抽出されたタスク候補」に追記→翌朝ohayoで草川承認後のみ `td.py add`） |
...
- 完了タスク（Todoist）→ `mcp__claude_ai_Todoist__find-completed-tasks` で今週月〜金の完了分を取得（td.py に完了履歴コマンドが無いため週次のみMCP使用。旧✅タスクDBはクエリしない）
...
📊 今日の実績
✅ タスク完了 N件 ／ 🚚 明日へ持ち越し M件（⚠️期限超過 K件・期限据え置き→朝morningで判断）
...
| ✅タスクDB（**廃止**・過去データの参照のみ可・新規書込/定常クエリ禁止。現役はTodoist） | `292cf503-a68f-81c6-b9dd-000b3ffdd2ce` |
| 🗂️プロジェクトDB（**廃止**・過去データの参照のみ可・新規書込禁止。現役はTodoistプロジェクト） | `292cf503-a68f-81fe-bd40-000b64314f2e` |
...
### 目的
2026-05-05のNotion完全棚卸しで発見した**旧情報DB(325dd77f-)配下100件超の未整理ページ**を毎晩10件ずつ漸進整理する。一気に処理せず**1日10件ペース**で約34日かけて完了。

### 対象データソース
1. **旧情報・質問DB** (`collection://325dd77f-2689-4d0e-ba79-f2a7d559014f`) — 100件超未処理
2. **その他📦旧_リネーム済DB配下の個別ページ**で未移行のもの
...
### KPI
- 1日10件ペース → 100件で**約10日完了**（2026-05-15頃）
...
## 📌 AI議事録からの市民意見 自動抽出（2026-07-05新設）
Step群でAIミーティングノート（当日分）を集約する際、議事録本文に**市民の声・要望・苦情**が含まれる場合：
1. 声を抽出し「🎙️ 意見候補（明朝承認待ち）」としてデイリーサマリに箇条書きで保存（1声1行・地区/会議体つき）。
2. **oyasumiでは市民意見リストへ直接登録しない**（完全自動運用のため誤登録防止）。登録は翌朝ohayoが承認を取ってから実行する。
3. 既に📝市民意見リストに同内容がある声は候補に含めない。
