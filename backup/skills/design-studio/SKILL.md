...
手順 — **参照画像と出力PNGの両方を自分でReadし、5軸＋採用則4項を数値・実物で比較する**（サブagentへ委譲しない）。

**A. 参照との模写5軸**（カルテの数値が目標）
...
**B. 造形の採用則**（`foundations/rules.md` §1.5・2026-08-25 追加。**Aを通っても、ここを割った紙面は未完成**）

| 項 | 可 | 不可（差し戻し） |
|---|---|---|
| ⑥ 写真の処理（§1.5-1） | **1面につき1か所以上**、角版以外がある（円形マスク／片側フェード／背景抜き／微傾斜） | 全ての写真が矩形（**v9三つ折りは9枚中8枚が角版**） |
| ⑦ 帯の形（§1.5-3） | 等幅の水平矩形でない帯が**表裏に1か所以上**（階段状／山形・曲線／フルブリードで回す） | 帯と罫線が全て水平矩形（**v9三つ折りの実例**） |
| ⑧ 質感（§1.5-4） | **選んだ様式カードの「質感」指定と一致**。文字の下に敷いていない／1面1種類 | 家庭用プリンタ物に質感を乗せた／文字の下に地紋・グレイン／2種類混在 |
| ⑨ 躍動（§1.5-5） | **選んだ様式カードの「躍動」指定と一致**。1面1角度＋写真の微傾斜まで | 角度3種類の混在／本文が乗る面・顔・水平線を傾けた／**`style_gov_formal`・`style_washu_calm` で角度を使った（この2つは躍動ゼロが正解）** |

**C. パレット整合**（`foundations/colors.html`）

- **1制作物1パレット**。ブランド4色（ライム`#c7ff4a`／濃緑`#1f5a3a`／最濃緑`#0f3d27`／生成り`#f3efe4`）と
  インク節約パレット（`#1f7a3a`／`#0e4d27`／金`#c89211`）を**混ぜたら差し戻し**
- **正本に無い緑を作らない**。`#0F7A3C`・`#8EC17C` のような「中間色」を :root に足したくなったら、それは様式選択か参照選択の誤り
  （**v9三つ折りは正本外の緑が5色入り、②色数が10まで膨らんだ**）

**合格ライン＝A は5軸中4軸で「可」以上、かつ B の⑥〜⑨とC は全項「可」**（BとCは1つでも割ったら差し戻し。
実装コストがほぼゼロで、割れていると素人紙面に直結するため例外を認めない）。
割った軸は print-layout-architect へ数値付きで差し戻し
（例:「①が1.5倍。参照は8倍。見出しを最低4倍まで上げる」「⑦帯が全て水平矩形。表紙のライム帯を幅違い2本の階段状に」）。
採点結果は毎回チャットに **A5行＋B/Cの割れた項だけ** で出す。
...
- 指示文に `ledger: <案件フォルダ>/_ledger.json` を1行入れる（テーマ内台帳。SNS告知など派生版は content-gate-lite 派生モードで継承）
...
5. **家庭用プリンタで刷る回はインク節約版を別途作る**（📌節「インク節約版」参照。CSS配色だけ差替・`_eco` 命名・両版保管）
...
## 📌 恒久ガードルール（このスキル常駐分）
- **④は「写真が要る中身か」を決めてから測る（2026-08-26追加・重大）**。数値を埋めるために未使用素材を探しに行かない。三つ折りi3面で、④4.91%を埋めるため未使用写真2枚を探して加工・キャプション・両ゲートまで通し、**草川の一言で全部破棄**した（写真が本文に合っていなかった）。④を割ったまま出すほうが正しい場面がある
- **写真は加工の前に「どこで・いつ撮ったか」を確認する（2026-08-26追加）**。造成地写真を「太岡寺町の用途地域変更区域」の前提で切り出しキャプションまで作ったが、市長答弁（造成未着手）と矛盾し、最終的に「そもそもその現場ではない」となって3往復した
- **工程の順序を固定する（2026-08-26追加）**：①中身→②事実確認→③文章の自然さ→④収まり→⑤安全ゲート→⑥デザインレビュー。**文言が動くたびにカラム幅の実測が無効になる**ので、文章を確定させてから収まりを詰める。安全ゲートは**主張が固まった時点で1回、確定時にもう1回**
- **削ったら必ずその場で通し読みする（2026-08-26追加）**。字数を詰めると①文末が切れる（「増やすよう。」）②常体と敬体が混ざる（「変えた。」）③主語のない終止形が連続する（「生かす。」「促す。」「広げる。」）が必ず起きる
- **市民の声は逐語引用か地の文の要約か、二択（2026-08-26追加）**。鉤括弧を外した要約を引用枠に入れる中間形は**合成引用として両ゲートに差し戻される**。ETLコーパスは末尾が「…」で切れていることがあるので、重要な引用はDriveの原本まで遡る
- **確認は柱・セクション単位でまとめる（2026-08-26追加）**。1文ずつ聞くと往復が数倍になる。AskUserQuestionは**判断だけの質問にpreviewを付けない**（previewを付けると左右分割UIになり自由記述欄が消える）
- **「破綻ゼロ」は合格ではない**（2026-08-12追加）。natural-design-reviewer が通しても、Step 4-B を割った紙面は未完成 — **A模写5軸（4軸以上）／B造形の採用則⑥〜⑨（全項）／Cパレット整合（全項）** の3段。帯と罫線だけの紙面・写真が全て角版の紙面・正本外の緑を足した紙面は、いずれも破綻ゼロで出荷されうる
...
- **インク節約版（家庭用インクジェットで刷る回・2026-07-18 川合町版レポートで確立）**。ネットプリント用フル配色版とは別に、**CSSの配色だけ**を差し替えた版を作る（本文・レイアウト・fact/risk承認済み内容は一切変えない＝再検証不要）。Why: 家庭用インクジェットは大面積のベタ塗り・濃色地でインクを大量消費し、実費とヘッド負担に直結する（レーザー/オフセットなら不要）。差替え内容＝①背景 warm paper `#fbfaf6` → 白 `#ffffff`（全面ウォッシュが消えるのが最大の節約）②淡ウォッシュのカード地（wash-green/wash-gold）→透明。分節はヘアライン＋余白で保つ ③大面積の濃色ベタはアウトライン化。特に円形ヌメラル（濃緑の塗り円）は緑リング＋白地＋緑文字 ④写真は残す（情報価値優先。写真がインク最大消費源である点は認識）⑤小面積のベタ（金タブ旗・セクションバッジ・タグラインピル・討議資料バッジ）は視認性のため残してよい。ファイル名は `..._eco.html` / `..._eco.pdf`、両版を保管。eスポーツ等のスポンサーロゴは節約版用の白地版（`gray/`）を使う（memory/project_esports_sponsor_logos.md）
- イベント運営段取り表の作り方の正本は `design_system/templates/runsheet_mobile/README.md`（幅390px・1ページ縦長PDF・ページ高さ実測・個人情報注意書き）