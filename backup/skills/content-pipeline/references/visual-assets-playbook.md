...
### A-0. 元写真の選定（最初の一手・必須／2026-09-04 改訂＝聞かずに選ぶ）
**草川に写真の有無を聞かない。** photo-curator を起動してこちらで選び切る（ZPERSON=18）。
毎回聞かれるのが手間だという指摘を受けての変更（2026-09-04）。旧운用＝[[feedback_thumbnail_ask_base_photo_first]] は破棄。
- **原則**: 写真を主役に据え、文字は写真の余白／スクリム上に。人物が主役だと訴求力が高い。
- **写真が見つからない／テーマに合うものが無い**ときだけ、タイポ＋モチーフ主体（下記テンプレ）に落とす。判断は自分で行い、事後に一行で報告する。
- 記事のテーマに当事者（子ども・妊婦等）が写り込む写真は、許諾と配慮の問題が出るため除外する。
