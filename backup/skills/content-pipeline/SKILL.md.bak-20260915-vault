...
description: "任意の素材（録音/文字起こし/テキスト/記事URL/写真）からブログ＋SNS7種＋条件でサムネ・Reelsを一括生成し、安全ゲート後にNotion・outputsへ保存。Triggers: 発信一式/発信セット/一式作って/フル展開して/記事から発信/ブログもSNSも/サムネ作って/コンテンツパイプライン/録音をアップした。NOT: 種1つ→spark"
---
...
> **時期（2026-09-15改定）**: 2.6→2.7 は草川がObsidianで注記1回目を入れた後に回す。結果は原稿の該当箇所直下に `> [!check]`／`> [!warning]` 囲み枠で書き込んだ次版として返し、本文は削らない→草川が注記で採否→「完成」時は変わった文・新規事実だけ再確認→gate.py→HTML→Notion1回。正本＝OPERATIONS.md【C】D6。
...
Agent(subagent_type="content-fact-checker",
  ...
  research_summary: {policy_researcher_result + kameyama_researcher_result}
  theme: {theme}
  ledger: ~/outputs/{日付}_{テーマ}/_ledger.json
...
## ステップ 3.6: 派生ゲート（SNS7本・必須・1テーマ1回）

ブログ（元版）は 2.6→2.7 の2段ゲートを通っている。SNS7媒体は**派生版**なので、content-fact-checker と content-risk-reviewer を別々に起動せず、**content-gate-lite の派生モード1回**で束ねる（2026-09-05 設計書§3-E）。ステップ4（サムネ・動画）が発動する場合は、**ステップ4の生成物が出揃ってから**サムネ文言・動画台本も同じ束に入れて1回にする（4-C参照）。発動しない場合はここで走らせる。

```
Agent(subagent_type="content-gate-lite",
  prompt="派生モードで、以下の派生版を1回で束ねて検証してください。agents/content-gate-lite.md の『派生モード』節に従う。
  mode: derivative
  ledger: ~/outputs/{日付}_{テーマ}/_ledger.json
  base: {ブログ確定稿のパス}
  content_type: sns-bundle
  theme: {theme}
  drafts:
  {sns_content_after_quality_review（媒体名を見出しに全文）}
  {サムネ文言・動画台本があれば続けて}
  判定: APPROVE / REVISE / ASK_USER / REJECT")
```

600字を超える媒体（長文Facebook等）は REROUTE が返る→その媒体だけ 2.6/2.7 と同じ2段に回す。

**判定に応じた分岐**:
- **APPROVE** → ステップ 5 に進む
- **REVISE** → sns-content-creator に修正指示を渡して該当媒体のみ再生成→同じ派生モードで再走行（差分のみ）
- **ASK_USER（HIGH検出）** → **草川に問いかけて判断を仰ぐ**。ユーザー応答後に次ステップ進行を決定
- **REJECT（CRITICAL検出）** → **即停止**。保存進めず、該当媒体を差し戻し or 削除

`_ledger.json` は 2.6/2.7 の終了時に SubagentStop フック `fact_ledger_autolog.py` が自動生成する。そのために **2.6/2.7 の指示文に `ledger: ~/outputs/{日付}_{テーマ}/_ledger.json` の1行を入れる**（親が書くのはこの1行だけ）。
...
### 4-C. 安全ゲート（サムネ・動画とも）
- 画像・動画に載る**文言**は別起動せず、**ステップ3.6の派生ゲート（content-gate-lite 派生モード）に束ねて1回**で通す（`ledger: ~/outputs/{日付}_{テーマ}/_ledger.json` を渡す。ブログと同一事実・同一表現は台帳から継承し、圧縮で但し書きが落ちていないかを必ず見る）。動画/画像独自の新規主張・人物写真の写り込み（他者の顔・実名のぼり以外の第三者）のみ重点確認。公選法の寄附/おもてなし表現・他議員名なしを最終確認。
