...
description: コンテンツパイプライン。録音・文字起こし・テキスト・ニュース記事URL・イベント情報・写真など任意のインプットから、ブログ記事・SNS投稿文7種（X/Threads/Instagram/Facebook/LINE/YouTube/TikTok）に加え、条件に応じてブログのサムネ画像（アイキャッチ/OGP 1200×630）とインスタ用ショート動画（Reels 9:16）まで一括生成し、安全ゲート通過後Notion・outputsに保存する。ブログ執筆モード（深掘り/ノーマル）は素材から自動判定して提案。「録音をアップした」「文字起こしがある」「テキストからブログとSNSを作りたい」「この記事からブログ作って」「この記事からブログとサムネとSNS作って」「サムネ作って」「アイキャッチ作って」「インスタ用ショート動画作って」「この記事から発信一式」「コンテンツパイプライン」等、既存素材から複数フォーマットへの変換・発信物一式の制作が求められる場合は、スキル名が挙がらなくても積極的に発動する。
---
...
該当時は**何を作るかを1行で確認**（例:「アイキャッチ用サムネ(1200×630)とインスタ用ショート動画(9:16)も作ります。よいですか？」）してから、`references/visual-assets-playbook.md` の手順で制作する。要点のみ再掲（詳細は必ず playbook を読む）:

### 4-A. サムネ画像（ブログのアイキャッチ / OGP / SNSカード）
1. **最初に「元になる草川の写真はありますか？」と1回確認**（[[feedback_thumbnail_ask_base_photo_first]]）。写真あり→ photo-curator でベスト選定して写真主役レイアウト／写真なしを草川が選択→タイポ＋モチーフ主体。
2. HTML/CSS→Chrome headless→PNG（1200×630・2倍解像度）。プロ級和文タイポ（ヒラギノW8/W9・`font-feature-settings:"palt"`・草川カラー #1f5a3a/#e0357a）。絵文字禁止（[[feedback_no_emoji_ai_smell]]・[[feedback_thumbnail_pro_typography_default]]）。
