...
**草川好みのセリフ構成（2026-06-02反映）**: ①最強フック1.5秒 → ②**名乗り**（※「撮っている理由」の一言は2026-09-03 草川指示で**廃止**。「だから、撮っています」型は今後一切書かない） → ③本論1メッセージ → ④**結びの決意「あきらめません／必ず実現させたい」**（※実現主体は市。議員個人が実現を断定約束しない＝[[feedback_sns_no_action_promise_assertions]]。2026-08-05 修正） → ⑤コメント誘発。尺は**35〜50秒・目標45〜50秒厳守**（名乗り＋決意込みでも50秒以内）。冒頭の名乗りはコールドオープン回避のため**フックの後ろ**に置く。詳細 [[feedback_short_video_use_virality_architect_first]] [[feedback_kusagawa_short_video_script_style]]。
...
- **会話の流れで台本を直し続けた案件は、スキルが一度も起動していないことを疑う。** 台本に「agent未起動・手動検証」「未伝播」と書かれていたら、それが未起動の痕跡。**その時点で Step 3〜5 を回収する**（fact-checker / risk-reviewer / 挿入画像）
