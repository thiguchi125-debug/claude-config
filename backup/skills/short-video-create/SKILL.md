...
- **テーマ または セリフ全文**（どちらか）
  ...
  - セリフ全文 → そのまま使う
- **target_length**: **35〜45秒（既定・厳守）**／60秒・90秒は草川案件では使わない
...
**草川好みのセリフ構成（2026-06-02反映）**: ①最強フック1.5秒 → ②**名乗り**（※「撮っている理由」の一言は2026-09-03 草川指示で**廃止**。「だから、撮っています」型は今後一切書かない） → ③本論1メッセージ → ④**結びの決意「あきらめません／必ず実現させたい」**（※実現主体は市。議員個人が実現を断定約束しない＝[[feedback_sns_no_action_promise_assertions]]。2026-08-05 修正） → ⑤コメント誘発。尺は**35〜45秒厳守**（名乗り＋決意込みでも50秒以内）。冒頭の名乗りはコールドオープン回避のため**フックの後ろ**に置く。詳細 [[feedback_short_video_use_virality_architect_first]] [[feedback_kusagawa_short_video_script_style]]。
...
| 結び（「まず◯◯／そして◯◯」型の2段まとめ） | ○ |
| サムネ文字案 | ○ 1枚。ただし着手前に元写真の有無を1回確認（[[feedback_thumbnail_ask_base_photo_first]]）。写真なしならタイポ版を作り、撮影後に差し替え可能な構造にしておく |
...
**6字/秒**で計算する（architect・memoryと統一。8.4字/秒は過小見積もりで60秒級を生む原因だった。2026-09-03修正）。字数が増えたら**必ず草川に申告**し、削る候補を1〜2個添える。黙って45秒を超えない。
...
- **セリフ**: ①コールドオープンのフック → ②名乗り（撮っている理由は書かない・2026-09-03廃止） → ③本論1メッセージ → ④結びの決意「あきらめません／必ず実現させたい」 → ⑤コメント誘発。**35〜45秒**（データ系で本論のビート数が多い回は `<!-- FORMAT-EXCEPTION: 尺=NN秒 / 草川承認 / 理由 -->` を原稿に書けば≤60秒まで可。2026-08-05追加）。※「撮っている理由」ブロックは廃止（2026-09-03 草川指示）。
