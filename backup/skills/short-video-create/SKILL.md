...
**草川好みのセリフ構成（2026-06-02反映）**: ①最強フック1.5秒 → ②**名乗り＋「最近このご意見がめちゃくちゃ多いんです」**（＝撮っている理由） → ③本論1メッセージ → ④**結びの決意「あきらめません／必ず実現させたい」**（※実現主体は市。議員個人が実現を断定約束しない＝[[feedback_sns_no_action_promise_assertions]]。2026-08-05 修正） → ⑤コメント誘発。尺は**35〜45秒厳守**（名乗り＋決意込みでも50秒以内）。冒頭の名乗りはコールドオープン回避のため**フックの後ろ**に置く。詳細 [[feedback_short_video_use_virality_architect_first]] [[feedback_kusagawa_short_video_script_style]]。
...
**8.4字/秒**で計算する。字数が増えたら**必ず草川に申告**し、削る候補を1〜2個添える。黙って60秒を超えない。
...
- **セリフ**: ①コールドオープンのフック → ②名乗り＋撮っている理由 → ③本論1メッセージ → ④結びの決意「あきらめません／必ず実現させたい」 → ⑤コメント誘発。**35〜45秒**（データ系で本論のビート数が多い回は `<!-- FORMAT-EXCEPTION: 尺=NN秒 / 草川承認 / 理由 -->` を原稿に書けば≤60秒まで可。2026-08-05追加）。※②の理由は事実に基づくこと（市民の声起点でない案件で「この声が多いんです」と言わない）。
