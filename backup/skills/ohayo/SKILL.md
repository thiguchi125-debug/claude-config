...
各項目の実行後にチェック。スキップは「⊘ skipped: 理由」を記録し、未実行があればチャット末尾に「⚠️ N件未実行」。全13完了後に §F（ダッシュボード3セクション更新）→ 最終出力。
...
スマホからの一次保存先は **Todoist Inbox**（考えずに放り込む場所）。朝ここで振り分けて空にする。`td.py list Inbox` を1回実行し、0件なら `✅ Inbox空` の1行で終える。1件以上あれば **1件1問**（AskUserQuestion・1回最大4問）で次の4択を聞く：
