---
name: ohayo
description: "朝のブリーフィング。「おはよう/おはよ/morning/朝のブリーフィング/今日のタスク/今日の予定教えて」で必ず起動。Calendar・Gmail・Todoist・Notionを横断し今日1日を日本語でまとめる"
---
...
13. 🗂 市民要望管理台帳の⚠要対応（§9b・`yobo.py alert` ローカル1call）
14. 📥 **Todoist Inbox棚卸し**（§5・**この項目だけは対話の往復が要る＝真っ先に劣化する**。一覧表示と「振り分けますか？」の文章は実行に数えない。0件でも「✅ Inbox空」の1行を必ず出す）

各項目の実行後にチェック。スキップは「⊘ skipped: 理由」を記録し、未実行があればチャット末尾に「⚠️ N件未実行」。全14完了後に §F（ダッシュボード3セクション更新）→ 最終出力。
...
### 📥 Inbox棚卸し（2026-09-05新設・必須・Bash 1call）

> ⚠️ **この節は2026-09-08・09-09・09-10と3日連続で飛ばされた。**原因は §0 の項目に入っていなかったこと（自己点検に引っかからなかった）。2026-09-10に14項目目として独立させた。飛ばしたら「⚠️ 1件未実行」を必ず出すこと。**一覧を出して「振り分けますか？」と文章で聞くのは実行ではない。**AskUserQuestion で選択肢を出すまでが1セット。
>
> **完了したら `ohayo_state.json` の `inbox_swept_date` に当日日付を書く**（0件で終えた日も書く）。翌朝この値が前日でなければ、ブリーフィング最上部に `🚨 昨日のInbox棚卸しが未実行（最終実施 YYYY-MM-DD）` を出す。


スマホからの一次保存先は **Todoist Inbox**（考えずに放り込む場所）。朝ここで振り分けて空にする。`td.py list Inbox` を1回実行し、0件なら `✅ Inbox空` の1行で終える。

**聞き方＝「振り分け先×期限」の表を1枚出し、草川が行ごとに直す（2026-09-15草川指示・恒久）**。AskUserQuestionで振り分け先だけを1件ずつ聞く方式は廃止（期限が抜け、昨日期限のまま移動→期限超過に埋もれた）。
- 先に `td.py nodue` と `td.py morning` で**各件の現在の期限**を確認し、「スマホ投入で昨日の期限が付いている」等を表の前に1行で書く
- 表の列＝`Inbox｜振り分け先｜期限案`。関連するメモ（同じ議会論点など）は1行に束ね、「既存タスクへコメント転記して閉じる」を振り分け先に使ってよい
- 期限案は必ず全行に入れる（今日・明日、または突合前の仮日付）。**明後日以降の期限は設定前にカレンダー突合**（task-add手順）すると添える
- 草川の返事は「〇〇は今日確認」「△△は質問メモにも」のような自由文で来る。返ってきた行だけ反映し、**返事が無かった行だけ**を最後に1問で聞き直す
- 期限の反映：今日・明日は `reschedule-tasks`、明後日以降は `_verified.json`→突合。台帳行は `yobo.py add --due`

振り分け先の選択肢（表に書く値）：

1. **台帳へ** → `python3 ~/.claude/scripts/yobo/yobo.py add --t 件名 --s 相談者 --a 相手先 --na 次アクション` の後 `td.py done <id>`（Todoist側は完了で閉じる・複製を残さない）
2. **箱へ** → `td.py mv <id> <PROJECT名>`（🏛議員活動／📝議会（会期別）／🎪地域・イベント／🗳選挙2026／📣発信／🏡家族・プライベート／🗂構想バックログ、またはその子）。期限が要るものは task-add の突合へ
3. **捨てる** → `td.py rm <id>`
4. **残す**（今日中に自分で触る）→ 何もしない

判定の物差しは1問だけ＝**「終わったら誰かに報告するか？」→はい＝台帳／いいえ＝Todoist**。件名の先頭が「要望：」なら台帳を第一候補に出す。相談者名がある件は台帳へ移した時点でTodoist側から氏名が消えるようにする（`done` で閉じる）。
...
2. `date` が今日（=今朝3:30実行分）→ `new_count` 0件なら「📮 ご意見箱: 新着なし」1行。1件以上なら「📮 ご意見箱 新着N件」として items の各行（行ID・地区・緊急度・1行要約・NotionURL）を表示。`urgent: true` があれば⚠️付きで最上部。末尾に「返信案が必要なら『◯番の返信案作って』（→ ikenフロー）」を添える。
   **タスク化候補も同時提示（2026-07-25追加・表示だけでは声が落ちる穴の締め）**: 対応アクションが読み取れる新着には、Todoist登録候補（内容・登録先プロジェクト案・期限案=今日+3日）を箇条書きで添え、「登録するものを選んでください（全部/番号/不要）」→**草川承認分だけ** `td.py add` で登録。**相談者がいて市（県・警察・自治会）と動く案件は Todoist でなく `python3 ~/.claude/scripts/yobo/yobo.py add` で市民要望管理台帳へ**（2026-09-05〜・「〇〇地区 市民相談」箱は全部アーカイブ済）。Todoist登録は草川自身の議会・政策・イベント作業だけ（🏛議員活動）。緊急度高は期限=今日〜明日を提案。
...
## §9b 市民要望管理台帳の⚠要対応（2026-09-05新設・必須・ローカル1call）
...
**締める直前に `td.py list Inbox` をもう一度叩き、0件（または草川が「残す」と言った分だけ）であることを確認してから出力する。** 残っていれば §5 の Inbox棚卸しに戻る。ここを通さずに最終出力を書き始めない。
