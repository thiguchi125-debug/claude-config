...
**展開ロジック**：
1. 1ミーティングから**最大5タスク**まで抽出（それ超過は切り捨て・ノイズ防止）
...
### Step 4.7: ニュースDBライフサイクル遷移（2026-07-03新設・軽量1クエリ）

📰ニュースDB（`29e5c1a2-`）の「ステータス=新着 かつ 日付が7日以上前」を1クエリで取得し、まとめて `確認済` へ更新（notion-update-page一括）。0件なら何もしない。「積むだけDB」化の防止＝新着は常に直近7日分だけになる。件数をデイリーサマリの数字欄に「🗞️ 新着→確認済 N件」と1行記録。
