---
name: oyasumi
description: 毎晩「おやすみ」「おやすみなさい」「good night」「夜のまとめ」「就寝」「1日の振り返り」「夜のブリーフィング」などと入力されたら必ず起動する。1日の活動（SNS投稿・ブログ・タスク完了・市民相談・ニュース・政策候補追加）を機械集約してNotionデイリーサマリを生成し、未完了タスクを明日に自動繰越し、明日のスケジュールをプレビューし、朝のダッシュボードに「昨夜のまとめ」セクションを反映する就寝時オートメーション。金曜日の夜のみ追加で週次レポートを自動生成する。完全自動化（途中で選択肢を求めない）。
---
...
#### 1-A. 明日のCalendar取得（2カレンダー並列）

`mcp__claude_ai_Google_Calendar__list_events` を2回並列実行：
- calendarId: `t.higuchi125@gmail.com`（プライマリ）
- calendarId: `kusakawa.taku@gmail.com`（**注：kusagawaではなくkusakawa**）
- time_min: 明日の00:00:00（ISO 8601）
- time_max: 明日の23:59:59（ISO 8601）

結果をマージして時刻順に並べ、タイトル・開始時刻・終了時刻・場所を抽出。

#### 1-B. 今日のCalendar振り返り取得

`mcp__claude_ai_Google_Calendar__list_events` を2回並列：
- calendarId: 上記同様2つ
...
|------------|--------------------|---------|
| SNS投稿管理DB | `1bd98deb-624f-402c-aeb3-bdaa4782b389` | `ステータス`が`投稿済`／`公開`／`完了`系 かつ 更新日=今日 |
| ブログ記事管理DB | `dcdf44af-f285-499e-a6c3-08cc343fc3fa` | `ステータス`が`公開済` かつ 公開日=今日 |
...
各DBで以下のいずれかを使用（**注：`notion-search` は data_source_url 指定でも0件返却することが多い・2026-04-30事故。下記の view_url 直接叩きを優先）**：

- ✅タスクDB（最重要）: `notion-query-database-view` を **必ず** 使う
  - view_url: `https://www.notion.so/292cf503a68f814eafcdec9eed56f273?v=292cf503a68f811e9555000c1fce8ad5`（「すべて」view・フィルタなし）
  - 取得後、**ローカルで以下を必ず実施**：
...
- 📰ニュースDB: `notion-query-database-view` で view_url `https://www.notion.so/f2eefc669dd54648bbcdacdc8afa1158?v=9d34f9a4-2bc1-4381-a31b-8f8c8ac719e2`
- その他のDB（SNS/ブログ/日次ログ/一般質問ネタ/市民意見/政策候補）: 各DBの「すべて」view URLを `notion-fetch` で確認した上で `notion-query-database-view` を使用

**0件でもエラー扱いせず、0件として記録する。**
