...
各DBで `mcp__claude_ai_Notion__notion-query-database-view` または `notion-search`（data_source_url指定）を使用。
...
Step 1-D で取得した「日次ルーティンログDB の今日のページ」が **存在しない** 場合のみ実行。

#### 自動推測ロジック

以下のソースから今日の活動を自動推測：
- 今日のCalendar予定（Step 1-C で取得したイベント）
- 今日のSNS投稿（1-D）
- 今日のブログ公開（1-D）
- 今日完了したタスク（1-D）
- 今日新規追加された市民意見（1-D）
...
**既に手動記録がある場合は何もしない**（上書き禁止）。

---

### Step 3: 未完了タスクの自動繰越し

Step 1-D で取得した「✅タスクDB（未完了・期限今日以前）」全件に対して：

#### 繰越しルール

- **期限が今日のタスク** → `期限` を **明日の日付** に更新（`mcp__claude_ai_Notion__notion-update-page` の `update_properties`）
- **期限が今日以前（=超過）のタスク** → タスク名先頭に `⚠️` プレフィクスを付ける（既に付いていれば追加しない）。期限は変更しない（超過の証跡を残すため）
- **`ステータス`が`inbox`のままで期限ナシ** → 何もしない
