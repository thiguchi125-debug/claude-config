...
本スキル内の「完了タスク集計・未完了タスクの繰越し・プロジェクト化判定」は **Notion✅タスクDB(292cf503)・プロジェクトDBではなく Todoist** を使う。下記表・Stepで `✅タスクDB` `292cf503` view を叩く記述は**無効**化し、代わりに：
- **今日完了したタスク** ＝ その日 `td.py done <id>` でcloseした分（会話・nichijoで完了にしたもの）。Todoistの完了履歴に依存せず、当日closeした件名を要約に載せる。
- **未完了タスクの確認・繰越し** ＝ `python3 ~/.claude/scripts/todoist/td.py overdue`（期限切れ）＋`td.py today`。繰越しは期限切れタスクを翌日へ送るなら `td.py add` ではなく既存タスクの期限付け替え（必要時のみ・基本は据え置きで草川が翌朝 `td.py morning` で再確認）。
