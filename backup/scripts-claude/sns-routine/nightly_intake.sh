...
if [ "$COUNT" -gt 0 ] || [ "$PENDING" -eq 1 ] || [ "$DOW" = "1" ]; then
  ...
  if "$CLAUDE_BIN" -p "$(cat "$DIR/pack_prompt.md")" \
      --allowedTools "Read,Write,Bash(grep *),Bash(python3 $DIR/discord_api.py *),mcp__claude_ai_Notion__*" \
      >> "$LOG" 2>&1; then
...
if [ -s "$VQ" ]; then
  ...
  if "$CLAUDE_BIN" -p "$(cat "$DIR/video_stage_prompt.md")" \
      --allowedTools "Read,Write,Edit,Bash(python3 *),Bash(grep *),Bash(cp *),Bash(mkdir *),Bash(/Applications/Google Chrome.app/Contents/MacOS/Google Chrome *),mcp__claude_ai_Notion__*" \
      >> "$LOG" 2>&1; then
