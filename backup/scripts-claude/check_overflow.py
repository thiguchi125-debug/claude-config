...
window.addEventListener('load', function(){
  ...
  // 横型キャンバスは LINE DIGEST の4:3中央クロップで文字の頭が欠けないか（2026-09-22 追加。
  // 機械チェック全通過→feed-visual-reviewerで毎回差し戻し、を止める）。文字のインク幅で測る
  if (W > H) {
    var c43 = (W - H*4/3)/2, M = 10;
    els.forEach(function(e){
      Array.from(e.childNodes).forEach(function(n){
        ...
        Array.from(rg.getClientRects()).forEach(function(r){
          if (r.width<1) return;
          if (r.left < c43+M || r.right > W-c43-M)
            out.push({t:'crop43', el:(e.className||e.tagName), txt:n.textContent.trim().slice(0,12),
                      left:Math.round(r.left), right:Math.round(r.right),
                      band:Math.round(c43+M)+'-'+Math.round(W-c43-M)});
        });
      });
    });
  }
