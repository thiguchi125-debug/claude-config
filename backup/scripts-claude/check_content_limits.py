...
def check_sns(text, single_pf=None):
    ...
    for sec in re.split(r"\n## ", src)[1:]:
        name = sec.split("\n")[0].strip()
        raw  = "\n".join(sec.split("\n")[1:])
        # 2026-09-06: URLは本文でもタグでもない（気象庁URLの "#pro&disp=" をハッシュタグと数え、
        # 約400字のURLを本文字数に含めていた）。字数・タグ判定の前に落とす
...
def check_video(text):
    ...
    # 2026-09-10 草川指示：締めを決意文句で固定しない。要求先＋要求内容まで書く。
    # 旧実装は「必ず実現／あきらめません」を必須にしていたため、3案件が同じ締めになっていた。
    out.append((bool(re.search(r"(てほしい|ほしい|働きかけたい|求めたい|訴えたい|提案したい)", _serifu)),
                "結びの主張＝どうしたいか（要求先＋要求内容・憲法・出荷拒否）"))
    out.append((not re.search(r"(あきらめません|必ず実現させたい)", _serifu),
                "決意文句の型固定（2026-09-10 廃止）"))
    ...
    # 2026-09-22: スキャン対象を「### カット表」節だけに限定。以前は body 全体を
    # 舐めていたため、後続の「挿入画像の配置マップ（CapCut用）」節が同じ時刻を
    # 重複記載しているとカット数が2倍になり、さらに帯の重ね秒数を説明する地の文
    # （例:「0:32.1〜0:47.6 の約15.5秒」）まで1カットと誤検出して
    # 「3.0秒超のカット」を誤爆させていた（個別避難計画ショート動画で発覚）。
    _cutsec_m = re.search(r"###\s*カット表[^\n]*\n(.*?)(?=\n##\s|\Z)", body, flags=re.S)
    cut_section = _cutsec_m.group(1) if _cutsec_m else body
    tc = re.findall(r"(\d{1,2}):(\d{2}(?:\.\d+)?)\s*[-–—~〜]\s*(\d{1,2}):(\d{2}(?:\.\d+)?)", cut_section)
