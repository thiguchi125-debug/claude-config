...
EXEMPT_PAGES = {
    ...
    # news-briefing の内部台帳2ページ（2026-09-06 草川承認）。🌅朝のダッシュボードの子で
    # ohayo が読むだけ・公開経路なし。📰ニュースDBの個別記事ページは除外しない（ブログ/SNSへ流れる）。
    "391cf503a68f8194be35fec5aede8a5e",  # 📰今朝のニュースダイジェスト
    "391cf503a68f811088a7c5a70e6741c8",  # 📰dedupインデックス
}
...
EXEMPT_PARENTS = {
    ...
    "34ecf503a68f81b09275d34249f59cf6",  # 📅 週次レポート（金曜夜の週次の親）
    "394cf503a68f81269d49d2015f39d7b4",  # 🤖 AI作業環境ハブ（エージェント/スキル一覧・引き継ぎ手順＝内部運用文書のみ・2026-08-31草川承認）
    # nichijo 記録モードの日次ログ（2026-09-17 草川承認）。oyasumi デイリーサマリと同じ理由＝
    # 草川本人しか読まない内部ログで公開経路なし。台帳形式(800字)判定では散文の活動ログが常に
    # deny され、記録モードが構造的に機能していなかった。
    "f6d15ac0-73fa-414f-8242-03e8f06aa4e3",  # 📓 日次ルーティンログDB
}
...
def norm(t):
    ...
    t = re.sub(r"<[^>]+>", "", t)
    t = re.sub(r"[*`>#|=]", "", t)  # 2026-09-16: gate.py:43 と揃える（==蛍光マーカー==・URL末尾の== で誤deny）
    return re.sub(r"[\s　]+", "", t)
...
def approved():
    ...
    try:
        ...
        return set(), []
    # 2026-09-03: 期限判定をファイル単位→レコード単位に変更。
    # gate.py がマージ方式になり、1ファイルに複数セッションの記録が併存するため、
    # 先頭の generated_at だけで全部を無効化すると他セッションの記録を巻き込む。
    def _born(e):
    ...
    for e in d.get("approved", []):
        ...
        if b is None:
            continue
        ttl = e.get("ttl_minutes", d.get("ttl_minutes", 120)) * 60
        if now - b <= ttl:
            live.append(e)
    return {e.get("fp") for e in live if e.get("fp")}, [e.get("text", "") for e in live]


def exempt_page(tool_input):
    """更新先が内部運用ログのページなら True。

    update-page は page_id、create-pages は parent（親ページ）で判定する。
    親で判定するのは、夜間バッチが毎晩ID の変わる新規ページを作るため
    （デイリーサマリ・週次レポート）。この2つの親配下は公開経路を持たない
    内部ログなので、EXEMPT_PAGES と同じ理由で素通しする。
    """
    pid = re.sub(r"[^0-9a-f]", "", str(tool_input.get("page_id") or "").lower())
    if pid and pid in EXEMPT_PAGES:
        return True
    parent = tool_input.get("parent")
    if isinstance(parent, dict):
        parent = parent.get("page_id") or ""
    par = re.sub(r"[^0-9a-f]", "", str(parent or "").lower())
    return bool(par) and par in EXEMPT_PARENTS
