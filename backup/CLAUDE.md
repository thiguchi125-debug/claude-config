...
- **セッションを区切る（最大の効き目）**: 100回ごとで約53%、60回ごとで約66%の削減（2026-09-02実測ベース）。PostToolUseフック `context_budget_notice.py` が120K/200K/300Kで警告し、**300K超は100Kごとに鳴り続ける**（旧実装は300Kで沈黙し、実測セッションはそこから988Kまで無警告で約50M浪費した）。
...
- **巨大なツール結果を本体に入れない**: Notion MCP等は1回の返り値が60〜95Kになる。一度入ると以降の全呼び出しで再送され続ける（文脈300Kで67Kを受けると残り100回で+6.7M）。同フックが約30K超の結果を検出して警告する。大量取得はサブエージェント側で受けて要約だけ返させる。
