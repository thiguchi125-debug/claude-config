...
## トークン節約（2026-09-03 実測更新）
コストは「文脈サイズ × ツール呼び出し回数」でほぼ決まる。文脈は**減らない**ので、長いセッションほど1回の呼び出しが高くつく（※2026年8月の実測値はサブエージェント未計上のため過少。正しい実測は `token_report.py`）。
- **まずサブエージェントの本数を疑う（最大の効き目・2026-09-03実測）**: 消費の**68%はサブエージェント**側。`<projects>/<sid>/subagents/*.jsonl` に別置きされるため長らく計測から漏れていた（9/2＝289M→**835M**）。本体の文脈が小さいままサブ側で燃えるので⛔は鳴らない（05e764db＝本体111Kのまま232M）。Agent完了時に `context_budget_notice.py` が30Mごとに🧨で知らせる。重い調査は**本数を絞る・範囲を狭める**が先。
- **セッションを区切る（本体側の最大の効き目）**: 100回ごとで約53%、60回ごとで約66%の削減（2026-09-02実測ベース）。PostToolUseフック `context_budget_notice.py` が120K/200K/300Kで警告し、**300K超は100Kごとに鳴り続ける**（旧実装は300Kで沈黙し、実測セッションはそこから988Kまで無警告で約50M浪費した）。
- **警告が出たら `/kugiri`（区切りスキル）**: 成果物の保存 → 引き継ぎメモ `~/.claude/handoff/LAST.md` → `/clear` までを1パス。再開は「再開」の一言でよい（私が LAST.md を1回だけ読む）。いまの文脈は `python3 ~/.claude/scripts/ctx_now.py` で実測できる。**300K未満では切らない**（起動固定費73Kの払い損）。
- **巨大なツール結果を本体に入れない**: Notion MCP等は1回の返り値が60〜95Kになる。一度入ると以降の全呼び出しで再送され続ける（文脈300Kで67Kを受けると残り100回で+6.7M）。同フックが約30K超の結果を検出して警告する。大量取得はサブエージェント側で受けて要約だけ返させる（※サブは本体の文脈を守るだけで**安くはない**。本数と範囲は必ず絞る）。
...
- **画像を伴う反復作業はサブエージェントへ隔離**: デザイン実装ループ（print-layout-architect／natural-design-reviewer）・Chrome自動化は、スクショが親セッションに残らないようAgent側で回す（同上・逃がした先で燃えるので本数は最小限に）。
...
`~/.claude/handoff/LAST.md`（24時間以内のもの）の本文を冒頭に自動注入するので、草川が説明する必要はない。
