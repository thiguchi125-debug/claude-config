...
## トークン節約（2026-08-11〜）
- **画像は溜め込まない**: 同じPNGを2回Readしない。Chrome自動化・デザインレビューのスクショは1枚あたり約1,500トークンを**セッション終了まで占有**する。画像を扱う長時間作業は別セッションかサブエージェントへ隔離。
- **長い作業は途中で `/clear`**: opus[1m]は自動圧縮が効きにくく、ツール200回超のセッションはコンテキストが300K超に膨らむ。
