...
- **タスク別ガードルールの読込（必須）**: 作業に入る前に `~/.claude/GUARDRAILS.md` の該当節をRead → 発信【1】【2】／印刷物・動画・画像【3】／Notion書込【4】／議会業務【5】／市民対応・報告会【6】／基盤トラブル【7】／個別案件の事実【8】／タスク登録・日程【9】。安全ゲートの一部として実行する。
...
- **市役所対応の市民要望はTodoistでなくGoogleスプレッドシート「市民要望管理台帳」で管理（2026-09-05〜）**: 市民から頼まれて市（県・警察・自治会含む）と動く案件は `python3 ~/.claude/scripts/yobo/yobo.py add|update|list` で台帳（Drive `00_名簿・個人情報/市民要望管理台帳_v1`・1行＝1案件＝受付→照会→回答→相談者へ報告）へ。Todoistは議会・政策調査・イベント・選挙・自分の作業に限定。相談者の氏名は台帳にだけ書く。判定は1問「**終わったら誰かに報告するか？→はい＝台帳／いいえ＝Todoist**」。スマホの一次保存先はTodoist Inbox・翌朝ohayo §5で振り分け。 **詳細＝OPERATIONS.md【B】例外節**
...
- **議会公務会議の資料Notionページ配置（必須）**: 個別の会議体イベントページは期別議会フォルダでなく **📅ミーティングノートDB（`26d7848d-ad1a-4e84-806f-a8dbccb1872b`）配下**。**本文はObsidian vault `20_議会活動/<会期>_<会議名>/` が正本・Notionは一覧の行とリンクだけ（2026-09-15）**。**詳細＝OPERATIONS.md【D】**
...
- **4つの置き場（2026-09-15）**: 渡す・スマホで使う完成品と原本→Drive／状況を追う記録→Notion（本文はリンク）／草川が読んで注記する文章→**Obsidian vault**（発信＝`50_発信/<日付>_<テーマ>/`）／それ以外→ローカル。正本＝vault `10_ClaudeCode/設定/置き場の使い分け.md`
- AI内部の下書き（草川が読まない物）→ `~/.claude/projects/-Users-kusakawatakuya/drafts/`／スキル生成物（画像・HTML）→ `~/outputs/`（絶対パス）
...
## トリガー早見
スキル・エージェントの起動語は各 description（起動時に自動読込）が正本。スキル外の2本＝「写真ストック整理して」→Drive `📷写真ストック/README.md`／「後援会取り込み」→Drive `06_フォーム・アンケート運用/後援会入会フォーム/README.md`。休眠スキル（drive-intake等）は「〇〇戻して」。一覧＝Notion「スキルトリガー一覧」「エージェントトリガー一覧」（MEMORY.md参照）。
...
- **仕組みの不具合は見つけたらその場で直す（2026-09-15改定・日曜枠は廃止）**: 草川「基本的にすぐやるべき」。日曜にまとめる運用は自動化されておらず、積んだ13件が9日間放置された。壊れた・ずれたと分かったら、その作業の中で**その1件だけ**直して動作確認し、結果を1行で報告する（Todoistに積まない）。燃費の歯止めは「範囲を広げない」側で掛ける＝ついでの改善・大改修（1件で複数ファイル群を作り直す規模）は提案に留め、草川の了承後に着手。
