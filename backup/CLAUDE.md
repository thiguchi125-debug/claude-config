...
- **仕組みの改修は週1枠（日曜・1セッション）に閉じる（2026-09-06）**: 8/23〜9/6の実測で改修3日（9/2〜9/4）が14日分の41%（2.1G）を消費。フック・スクリプト・スキル・エージェント定義の修理や改善は日曜にまとめる。平日に壊れたら手動で凌ぎ、修理は `td.py` に「@日曜改修」で積む。例外は草川が「今直して」と言った時だけ。
